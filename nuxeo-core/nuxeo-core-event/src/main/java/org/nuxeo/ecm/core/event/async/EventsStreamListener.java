...
public class EventsStreamListener implements EventListener, Synchronization {
    ...
    public void handleEvent(Event event) {
        ...
        ofNullable(event.getContext().getPrincipal()).map(NuxeoPrincipal::getActingUser).ifPresent(builder::withUsername);
    ...
    protected void appendEntries() {
        ...
        txId = TransactionHelper.lookupTransactionManager() == null
        entries.get().forEach(event -> {
            // Append the record to the log
            byte[] encodedEvent = codec.encode(event);
            appender.append(key, new Record(event.id, encodedEvent, Watermark.ofTimestamp(event.getTime()).getValue()));
        });
    }
